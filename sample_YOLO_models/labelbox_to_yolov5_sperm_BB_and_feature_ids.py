...
def labelbox_to_yolov5(project_id, api_key, output_dir):
    ...
    for label_object in project.video_label_generator():
        ...
        video_label_output_path_with_ftid = os.path.join(output_dir, video_name, "labels_ftid")
        ...
        if not os.path.exists(video_label_output_path_with_ftid):
            os.makedirs(video_label_output_path_with_ftid)
        ...
        video_labels_with_feature_ids = defaultdict(list)
        ...
        for label in label_object["annotations"]:
            ...
            feature_id = label["extra"]["feature_id"]
            ...
            video_labels_with_feature_ids[frame_id].append([feature_id, class_id, norm_center_x, norm_center_y, norm_label_width, norm_label_height])
        ...
        for frame_index, frame_labels in video_labels_with_feature_ids.items():
            # saving frame annotations with feature ids
            frame_save_path_ftid = os.path.join(video_label_output_path_with_ftid, "%s_frame_%i_with_ftid.txt" % (video_name, frame_index-1))
            with open(frame_save_path_ftid, "w") as f:
                for frame_label in frame_labels:
                    f.write(" ".join([str(val) for val in frame_label]) + "\n")
            f.close()
